...
module.exports = function (grunt) {
  ...
  grunt.config.init({
    ...
    preprocess: {
      build: {
        files: {
          'dist/colorful.js': 'src/colorful.js'
        }
      },
      interactive: {
        files: {
    ...
    concat: {
      ...
      build: {
        src: 'dist/colorful.js',
        dest: 'dist/colorful.js'
      }
    },
    ...
    // Use `concurrent` to run multiple continuous, blocking task simultaneously.
    //
    // Use cases:
    //
    // - You can combine a `watch` or `focus` task, which runs forever, with other non-ending tasks. One such task is
    //   `compass`, which provides an independent watch functionality of its own. It runs forever if that watch is used.
    //
    // - You can combine a task like `weinre`, which runs forever, with a task which must not start before it, e.g.
    //   `open` (opening a browser and navigating to the Weinre page).
    //
    // NB:
    //
    // - `concurrent` is not needed for running `watch` with multiple targets - use `focus` for that.
    // - Compass builds could be made part of a Grunt task which is kicked off by Grunt `watch`, rather than Compass'
    //   own watch functionality. The setup would be easier because `concurrent` is not needed then. But that setup has
    //   proven to be a major annoyance because it is very, very slow indeed.
    concurrent: {
      options: {
      ...
      // `weinre`/`open` example
      debug: {
        tasks: ['weinre:debug', 'open:debug']
      },

      // `focus`/`compass` examples
      demo: {
        tasks: ['compass:watchDemo', 'focus:demo']
      },
      demoCi: {
        tasks: ['compass:watchDemo', 'focus:demoCi']
      },
      demoCiDirty: {
        tasks: ['compass:watchDemo', 'focus:demoCiDirty']
      }
    },
