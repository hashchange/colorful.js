...
- `vendor.js` for the third-party dependencies. It includes Colorful.js.
- `app.js` for the demo code, consisting of local modules.

The code is not rolled up into a single file because that file would be massive, making it unnecessarily difficult to examine the demo code. The purpose of the demo is to see how Colorful.js is used, so it makes sense to keep the client code separate.
